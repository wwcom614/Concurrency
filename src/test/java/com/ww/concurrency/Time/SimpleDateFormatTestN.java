...
import com.ww.concurrency.annoations.NotRecommend;
...
@NotThreadSafe
@Slf4j
public class SimpleDateFormatTestN {

    //请求总数
    public int requestTotal = 5000;
    //允许同时并发执行的线程数
    public int threadNum = 200;
    //全局静态变量，多线程测试，结果是线程不安全。多线程示例运行时会抛出大量异常。
    //public static SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
    // 所以基于线程封闭方法，将SimpleDateFormat改为局部变量使用
    ...
    private void dateTransform() {
        try {
            SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
            simpleDateFormat.parse("2018-05-31");
