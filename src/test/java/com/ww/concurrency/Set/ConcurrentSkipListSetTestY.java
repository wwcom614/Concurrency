...
/*ConcurrentSkipListSet基础操作是线程安全的。不允许有null
但带all的操作是线程不安全的，例如addAll,containsAll,removeAll等，此时需要加锁
先开辟一块空间复制数据写，然后将原数据引用指向新的这块空间
读写分离，最终一致性。对应排重排序的TreeSet
缺点：
...
@ThreadSafe
@Slf4j
public class ConcurrentSkipListSetTestY {
